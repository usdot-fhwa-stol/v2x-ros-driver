...
# Args:
#   $1: WSM forward profile (typical|filtered)
input=""
read_profile() {
  local val
  while :; do
...
if [[ $# -ge 1 ]]; then
  case "$1" in
    typical|filtered) input="$1" ;;
    *)   printf 'Error: arg must be "typical" or "filtered"\n' >&2; exit 1 ;;
  esac
fi
...
if [[ "$input" == "typical" ]]; then
  ...
  EXPECTED["RSU-MIB::rsuDsrcFwdPsid.3"]="STRING: 80030000"
  EXPECTED["RSU-MIB::rsuDsrcFwdPsid.4"]="STRING: 80100000"
  EXPECTED["RSU-MIB::rsuDsrcFwdPsid.5"]="STRING: bfee0000"
...
if [ $FAILED -ne 0 ]; then
    ...
    echo "Defaulting to WSM forward profile = filtered"
